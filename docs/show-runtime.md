...
Cursor travel, marker, underline, arrow, focus frame, relationship arrow, and native-selection reveal share one natural kinematic contract. Duration follows the measured smoothed arc length, so a large gesture takes longer than a small one. Moving phases use a 1.1 px/ms minimum, 2.0 px/ms target, 2.0 px/ms hard ceiling, and 220 ms minimum duration; acceleration and deceleration remain minimum-jerk rather than constant-speed. Distance is sampled by arc length. Adaptive geometric-tolerance subdivision and centripetal smoothing remove visible corners while preserving target coverage and make the result independent of the source curve's parameterization. Width varies smoothly with speed, curvature, and the derived pressure profile. String or numeric seeds pass through one stable normalization step; seeded irregularity is evaluated at normalized arc coordinates, is spatially smooth and deterministic, and never becomes per-frame jitter. Enclosing ink deliberately finishes with a laterally displaced overlap, so its tail remains visibly offset instead of forming a mathematically sealed loop.

Native selection progressively extends the actual browser `Selection`/`Range` or text-control range, derives duration from the measured selection travel, and restores the prior selection when requested. A consumer may author an exact quote/occurrence or explicit offsets; when it supplies only presentation intent and a resolved target, the provider selects the target's complete text instead of requiring the product to duplicate localized copy.

An action with an authored deadline passes `budgetMs`, `targetIdentity`, and, when available, `layoutIdentity` to `present()`, together with a callable `onAdmission`. Any request for the provider-v2 budget or callback contract requires that reporter; a missing reporter throws `SHOW_ATTENTION_ADMISSION_REPORTER_REQUIRED` before target planning, pixels, RAF ownership, or provider lifecycle state. Only a request with no provider-v2 inputs or callbacks uses the unbudgeted legacy presentation path. The controller first asks its cursor or selection adapter for an immutable zero-progress plan; this reads geometry but does not focus, select, render, move the presenter cursor, change ink, or request a frame. It then synchronously calls `onAdmission(providerAdmission)` with the one current `show-attention-admission-v2` shape. The admission has distinct frozen `provider`, `effect`, `target`, `budget`, `plan`, and `reason` namespaces: provider id/version cannot be confused with plan version, gesture id cannot be confused with plan identity, and exact zero-progress evidence remains nested under `plan.evidence`. Admitted plans require target, layout, geometry, and plan identities. A path effect requires `normalizedPathHash`; a truthful non-path click uses the empty string. Rejection retains both the stable reason and exact structured provider detail, leaves unavailable plan values `null`, calls admission synchronously, clears provider-owned transient state, and emits no visual milestone. `createShowAttentionAdmission()` is the Node-safe pure admission surface for consumers that already hold provider plan evidence.

After admission returns successfully, `onMilestone` receives the exact immutable `show-attention-milestone-v2` shape `{ version, milestone, observedAt, admission, providerReceipt }` for `first-frame` and `settled`. `observedAt` is always `{ domain: "performance", timeOriginMs, monotonicTimeMs }`; animated effects use the actual RAF callback timestamp and immediate, hostless, and reduced-motion effects use `performance.now()`. The two synchronous milestones may share the same observation, but neither can precede admission and no epoch timestamp is substituted. The callback contains no Workspace status or identity; consumers map provider milestones outside the UI package.

`whenSettled()` resolves to the same exact `show-attention-terminal-v2` shape delivered once to optional `onTerminal`: `{ version, status, observedAt, admission, providerReceipt, timing }`. Provider status is only `completed`, `rejected`, `cancelled`, or `failed`; cancellation cause and first-frame timing remain nested in `timing`, while the last exact provider plan/render evidence remains in `providerReceipt`. Replacement, pause/resume, seek, cancel/Stop, reset, and disposal cannot replay a stale RAF or duplicate a milestone or terminal receipt. `pause()`, `resume()`, `seek()`, `captureState()`, `restoreState()`, `cancel()`, and `reset()` preserve deterministic replay. `prefers-reduced-motion` renders the final semantic state immediately after admission. Transient settled state remains visible until replacement or `clearTransient()`.
...
`ShowAudioArbiter` grants one audible source at a time. Audible media preempts and pauses speech before it begins. `ShowMediaController` exposes four canonical modes:
...
- `short-inline-continuous`: muted, controls hidden, not skippable by the Show control, with `pointer-only` interaction semantics. It returns after playback begins and remains active until stopping, ending, or replacement.
...
- `spinner-rotation`: muted, controls hidden, not skippable by the Show control, with `pointer-only` interaction semantics. Intended for 360-degree spinner targets whose rotation starts once and keeps playing until an explicit user interrupt, stop, or replacement; the target-owned completion settles only through abort.

Media directives may carry nonnegative `startMs`/`endMs` bounds and generic bounded-choreography data: strictly increasing `segments` fractions in the open interval `(0, 1)`, positive-integer `segmentDurationMs`, positive-integer `frames`, positive-integer `frameHoldMs` and `finalFrame`, and boolean `keepPlayingDuringQuote`. Normalization freezes arrays, converts finite numeric input to numbers, rejects empty, unordered, out-of-range, non-integer, or non-boolean values, and requires `endMs` to follow `startMs` when both are present. The discriminated JSON Schema requires each directive's canonical fields, rejects fields owned by another directive type, and couples the `show:*` envelope type to `directive.type`. It independently enforces non-empty arrays, unique in-range fractions, positive integers, and booleans. JSON Schema does not compare adjacent array values, so canonical ascending order is guaranteed by `normalizeShowDirective()` rather than raw schema validation.

In addition to native `HTMLMediaElement` targets, a target may expose `captureShowMediaState(context)`, `applyShowMediaPolicy(policy, options, context)`, `playShowMedia(options, context)`, `pauseShowMedia(reason, context)`, and `restoreShowMediaState(state, context)`. Existing hooks may ignore the additive argument. The immutable context is `{ operationId, signal }`; the same ownership identity reaches every hook for one operation. Hooks are optional, and an `element` property provides the native readiness, ended-event, state, and identity fallback. The provider resolves that native fallback once when `play()` is requested and pins the same object for capture, policy, playback, pause, ended-listener ownership, result reporting, and restoration; rebinding a wrapper's `element` during playback cannot redirect cleanup to another element. A custom hook continues to own its corresponding target operation. The policy passed to adapters is immutable and contains `mode`, `muted`, `semantics`, `nativeControls`, and `skippable`. Playback options are also immutable and contain only canonical `id`, `mediaId`, `mode`, and documented media directive fields; arbitrary consumer fields never cross the provider boundary.

Starting another operation, stopping, or replacement aborts the active signal with an `AbortError` before cleanup, so a bounded segment or frame sequence cannot continue after it becomes stale. Startup and teardown mutations are serialized: a successor cannot capture or apply policy until prior pause, audio release, and restore attempts finish. A late capture, policy, or playback settlement therefore cannot restore or mutate a replacement. Custom state hooks are an atomic pair: providing only `captureShowMediaState` or only `restoreShowMediaState` rejects before replacing active media, applying policy, or attaching an ended listener, so opaque state can never enter native restoration. `ShowAudioArbiter` retains one shared in-flight preemption barrier: every overlapping acquire waits until the old source's pause or stop attempt completes, then only the latest generation installs a lease. An exact-token stop requested in the same turn upgrades a pending pause-style preemption without cancelling its successor. A lease is returned only after its change notification settles; a rejected notification rolls back only that exact token and generation, independently attempts source cleanup and the null notification, and aggregates any failures. A stale acquisition rejects with `AbortError` without replacing or releasing the newer lease.

Stopping, ending, replacement, skip, or rejected playback restores the captured state. Pause, audio release, state restoration, and lifecycle notification are attempted independently; cleanup failures reject the explicit caller with `AggregateError`. Lifecycle notification return values are Promise-assimilated: a rejected start notification aborts and cleans up that operation, while terminal error-reporter rejection is consumed. Audio release likewise attempts both change notification and old-source pause/stop before aggregating failures. An ended-event cleanup failure is consumed and reported through `onEvent({ type: "show:media-error", phase: "cleanup", ... })`, avoiding a dangling rejection. Native media restores current time, mute, volume, playback rate, controls, and the captured paused state, including awaiting `play()` when the source had originally been playing; custom targets receive their opaque captured state unchanged. The former provisional name `full-with-skip` is not a public mode; consumers must use `full-with-media-audio`.
