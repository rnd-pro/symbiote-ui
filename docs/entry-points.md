...
- `symbiote-ui/control/transport` - Node-safe import guard and direct browser component entrypoint that exports and registers the host-driven `sn-transport` playback control when DOM globals are available.
- `symbiote-ui/chat/workspace` - Node-safe import guard and direct browser component entrypoint that exports and registers `chat-workspace` together with its transcript, message, composer, sidebar, and animated-background dependencies when DOM globals are available.
...
- `symbiote-ui/chat/show-runtime` - Node-safe, product-neutral narrated Show contract and runtime facade for typed events/directives, branch/return state, exclusive attention, accumulating markers, DOM/media readiness, meaningful-interaction pause, explicit resume, audio arbitration, media modes, restoration, and `workspace-aligned-sequence-v3` cue playback driven by `media.currentTime`. Consumers retain scenario content, target resolution, authored alignment production, routing, transport, and persistence; see `docs/show-runtime.md`.
- `symbiote-ui/chat/show-chat` - Node-safe provider helpers plus registry-first browser registration for `agent-dock-shell`, `agent-show-chat`, and `chat-show-player`. It reuses `panel-layout` and chat elements already registered by independently built application bundles without evaluating duplicate transitive component graphs. The compact player occupies a fixed region outside transcript scrolling and provides a material header, bounded current timeline, in-player caption/TTS blocks, detail/pointer-only video hooks, injected settings/actions, and transport controls. The Cascade-themed responsive shell keeps the same enabled chat, transcript history, contextual cards, and player mounted while it switches between a bounded desktop peer dock with resizer and a mobile drawer or stack. Scripted and API-backed agents share the single `respond(request)` provider boundary.
...
- `ChatMessageItem` / message-model custom-content parts - messages carry an `actions` part (`{ type: 'actions', actions: [{ id, label, icon, variant }] }`) that renders inline action buttons and emits `chat-message-action` / `chat-workspace-action` `{ id, actionId, payload }`, a reusable `footnote` aside (`{ type: 'footnote', text, url?, meta: { referenceId? } }`), and an `embed` part (`{ type: 'embed', key }`) that renders a keyed slot so `ChatTranscript` can emit `chat-transcript-embeds-ready` / `chat-workspace-embeds-ready` `{ embeds: [{ key, slot }] }` for the host to mount and re-attach a live widget.
