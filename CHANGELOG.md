...
- Extended the XR spatial-window assembly with the complete input and viewport seam: wheel and select-drag scroll relay to the live DOM with begin/update/end/cancel lifecycle, stable capture identity, derived grab deltas, per-window texture invalidation, and scroll offsets preserved across exit/re-entry (`xr-spatial-window-scroll-receipt-v1`); text selection drag through pointer down/move/up/cancel with stable capture and selection data read from the live DOM, never fabricated (`xr-spatial-window-selection-receipt-v1`); content focus, blur, and cancel for editable controls with real DOM focus and structured `dom-focus`/`dom-overlay` IME handoff data that never clones values or synthesizes text (`xr-spatial-window-focus-receipt-v1`); and a public per-window `updateWindowViewport` operation that updates the CSS viewport without remounting, preserves focus/form/selection/scroll, requests paint, and returns a transaction receipt with measured texture pixels (`xr-spatial-window-viewport-receipt-v1`). Resize commit now uses the same viewport operation and swaps texture and geometry on the same native mesh through the new public Three adapter `setPanelSize` per-panel operation, avoiding full-scene churn; resize commit and viewport receipts now report measured texture width/height.
- Added the public, product-neutral, SSR-safe XR spatial-window assembly (`symbiote-ui/xr/spatial-window-assembly` and `symbiote-ui/xr/spatial-window-contract`). One open layout instance maps to one XR window whose internal panel tree stays the live DOM content of that window. The assembly composes the existing HTML-in-Canvas renderer, panel host and input relay, Three/WebXR adapter and texture bridge, Meta-style external chrome, portable panel pose state, layout projection, cascade theme bridge, and frame timing primitives. It provides versioned layout descriptors, idempotent `syncLayouts` reconciliation, enter/exit/re-entry with session adopt/release and pose restore, focus and content-revision updates, controller/hand ray routing to external chrome or the live DOM relay, resize preview/commit/cancel with unscaled preview and transactional commit, explicit supported/unsupported/upload-failure fallback as data, dirty-gated uploads with zero idle frames, and per-window diagnostics. All receipts are data-only and schema-covered, and DOM/`THREE` dependencies are factory-injected so module evaluation stays Node-safe.
...
- Added a native held-audio stream contract to `VoiceRuntime` (`chat/voice-runtime.js`): `setHeldAudioStream(stream, { owned })` lets hosts reuse an already-authorized microphone stream for raw-audio capture without a second `getUserMedia` prompt. Ownership is deterministic — borrowed streams (default) are never stopped by runtime cleanup and stay held until replaced or cleared, while owned streams are stopped exactly once (on replace, clear, post-capture cleanup, or `destroy()`). A held stream without a live audio track rejects the capture start deterministically instead of silently falling back to `getUserMedia`; speech-recognition mode is unchanged. Stream cleanup now also releases recorder state on every cancel, stop, error, and dispose path.

### Changed

- `createXRPanelHost().mountPanel` now adopts a provided live element (`panel.element`) instead of always building a new one, and skips the container replacement when the element is already parented. This keeps live DOM state (drafts, scroll offsets) intact across viewport re-layout such as XR window resize commits.
...
- Fixed XR spatial-window default placement: unplaced layout descriptors no longer collapse onto the shared `XR_SPATIAL_WINDOW_DEFAULT_POSE` and overlap, and default placement is now a bounded deterministic capacity contract (`xr-spatial-window-placement-v2`). The Node-safe `symbiote-ui/xr/spatial-window-placement` lattice spans two user-facing radial arc tiers (1.96 m with four columns, 1.15 m with three columns) times a small set of vertical levels derived from the provider comfort envelope, so up to `XR_SPATIAL_WINDOW_PLACEMENT_CAPACITY` (24) default 0.8 m × 0.45 m windows are comfortable, above the floor, facing the viewer, and non-overlapping. Chord and level spacing adapt to each window's actual `sizeMeters`; when no safe non-overlapping slot exists the contract returns structured `placement-capacity-exhausted` data instead of an unsafe slot, and `syncLayouts` reports it as a per-layout error. Explicit descriptor poses still win byte-equivalently and become the reset canonical, batch placement resolves against all explicit poses in the batch before assigning defaults so input ordering cannot collide, stable re-syncs never move existing windows (including user-settled poses after drag), and slots freed by removal or drag are reusable. `resetWindowPose` and the chrome reset action are transactional: they restore a currently free canonical pose, re-resolve the lowest safe slot, or return a structured failure, and never overlap a live window. Window summaries and diagnostics expose live `defaultSlot` ownership (`null` for descriptor-posed windows and for windows that vacated their slot, so no duplicate live owners).
