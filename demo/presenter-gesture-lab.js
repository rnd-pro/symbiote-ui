...
const kinematicsCase = query.get('case') === 'long' ? 'long' : 'short';
const kinematicsSample = {
  name: `${kinematicsCase === 'long' ? 'Long' : 'Short'} seeded underline`,
  note: 'Arc-length timing, minimum-jerk motion, smooth deterministic variation, and variable-width ribbon.',
  annotation: { kind: 'marker', marker: 'underline' },
  className: `kinematic-${kinematicsCase}`,
  label: kinematicsCase === 'long' ? 'A deliberately long presenter trajectory' : 'Short cue',
  seed: 731,
};
const visibleSamples = focus === 'pointer'
  ? samples.filter((sample) => sample.annotation?.intent === 'pointer')
  : (focus === 'kinematics' ? [kinematicsSample] : samples);
...
function renderKinematicFrame({ progress = 1, seed = kinematicsSample.seed } = {}) {
  const target = document.querySelector('[data-index="0"]');
  const frame = cursor.presentAnnotationFrame(target, kinematicsSample.annotation, {
    progress,
    seed,
    viewport: { width: innerWidth, height: innerHeight },
  });
  document.body.dataset.kinematicProgress = String(progress);
  document.body.dataset.kinematicSeed = String(seed);
  document.body.dataset.kinematicReady = 'true';
  const evidence = {
    case: kinematicsCase,
    progress: frame.progress,
    durationMs: frame.durationMs,
    arcLengthPx: frame.arcLengthPx,
    averageSpeedPxPerMs: frame.averageSpeedPxPerMs,
    speedPxPerMs: frame.speedPxPerMs,
    minWidthPx: frame.minWidthPx,
    maxWidthPx: frame.maxWidthPx,
    normalizedPathHash: frame.normalizedPathHash,
    tailPolicy: frame.tailPolicy,
    pathPoints: frame.pathPoints,
    safety: frame.safety,
    targetRect: frame.targetRect,
    drawRect: frame.drawRect,
  };
  window.__presenterKinematicEvidence = evidence;
  document.body.dataset.kinematicEvidence = JSON.stringify(evidence);
  return evidence;
}

if (focus === 'kinematics') {
  window.presenterGestureLab = Object.freeze({ renderFrame: renderKinematicFrame });
  const requestedSeed = Number(query.get('seed') || kinematicsSample.seed);
  const initialFrame = renderKinematicFrame({
    progress: query.get('animate') === '1' ? 0 : Number(query.get('progress') || 1),
    seed: requestedSeed,
  });
  if (query.get('animate') === '1') {
    document.body.dataset.kinematicAnimation = 'running';
    const startedAt = performance.now();
    const animate = (now) => {
      const timelineProgress = Math.min(1, (now - startedAt) / initialFrame.durationMs);
      renderKinematicFrame({ progress: timelineProgress, seed: requestedSeed });
      if (timelineProgress < 1) requestAnimationFrame(animate);
      else document.body.dataset.kinematicAnimation = 'settled';
    };
  ...
  for (const [index, sample] of visibleSamples.entries()) {
    ...
    if (sample.focus) {
      const initial = cursor.presentFocusFrame(target, { mode: 'frame', elapsedMs: 0, viewport: { width: innerWidth, height: innerHeight } });
      const frame = cursor.presentFocusFrame(target, { mode: 'frame', elapsedMs: initial.durationMs, viewport: { width: innerWidth, height: innerHeight } });
