...
} from './NodeCanvasRenderSnapshot.js';
...
export class NodeCanvas extends Symbiote {
  ...
  /** @type {Object|null} */
  _lastPcbRouteSnapshotReceipt = null;
  ...
  _publishPcbRouteSnapshotReceipt(receipt) {
    this._lastPcbRouteSnapshotReceipt = receipt
      ? JSON.parse(JSON.stringify(receipt))
      : null;
    if (receipt && typeof CustomEvent === 'function') {
      this.dispatchEvent(new CustomEvent('node-canvas-render-snapshot-receipt', {
        detail: JSON.parse(JSON.stringify(receipt)),
        bubbles: true,
      }));
    }
    return this.getPcbRouteSnapshotReceipt();
  }
  ...
  capturePcbRouteSnapshot(routeFingerprint) {
    if (!this._connRenderer?.capturePcbRouteSnapshot) {
      throw new Error('NodeCanvas PCB route snapshot capture requires the SVG connection renderer.');
    }
    return this._connRenderer.capturePcbRouteSnapshot(routeFingerprint);
  }

  /**
   * @param {object} snapshot
   * @param {{routeFingerprint?: object}} [options]
   * @returns {object}
   */
  adoptPcbRouteSnapshot(snapshot, options = {}) {
    let receipt = this._connRenderer?.adoptPcbRouteSnapshot?.(snapshot, options);
    if (!receipt) {
      receipt = createNodeCanvasRenderSnapshotReceipt({
        adopted: false,
        reason: 'renderer-unavailable',
      });
    }
    return this._publishPcbRouteSnapshotReceipt(receipt);
  }
  ...
  invalidatePcbRouteSnapshot(reason = 'explicit-invalidation') {
    let receipt = this._connRenderer?.invalidatePcbRouteSnapshot?.(reason)
      || createNodeCanvasRenderSnapshotReceipt({ adopted: false, reason });
    return this._publishPcbRouteSnapshotReceipt(receipt);
  }

  /** @returns {object|null} */
  getPcbRouteSnapshotReceipt() {
    let receipt = this._connRenderer?.renderSnapshotReceipt
      || this._lastPcbRouteSnapshotReceipt;
  ...
  _readNodeElementSize(el, entry = null) {
    ...
    return readNodeCanvasLogicalSize(
      el,
      borderBox?.inlineSize || entry?.contentRect?.width || 0,
  ...
  measureNodeSizes() {
    ...
    for (const [nodeId, el] of this._nodeViews) {
      if (!el) continue;
      let cached = this._nodeSizeCache.get(String(nodeId)) || {};
      let { width, height } = readNodeCanvasLogicalSize(el, cached.width, cached.height);
      if (width > 0 && height > 0) {
        sizes[nodeId] = { w: width, h: height };
      }
    }
    return sizes;
  }
